...
class OmegaTrick_plugins extends xFrameworkPX_Controller_Action
{
    public function execute()
    {
        ...
        // 出力先ディレクトリ内のファイルを削除
        $ret = get_filelist($outDir . 'plugins/');
        $outDirFiles = array();
        foreach ($ret as $file) {
            if (!matchesIn($js, '.svn')) {
                $outDirFiles[] = $file;
        ...
        foreach ($jslist as $js) {
            ...
            $cmd .=  ' --js_output_file=' . $outDir . 'plugins/' . $file; 
        ...
        // 出力ファイル名設定
        $cmd .=  ' --js_output_file=' . $outDir . 'plugins-min.js';

        echo 'plugins-min.js 出力中...' . PHP_EOL; 
